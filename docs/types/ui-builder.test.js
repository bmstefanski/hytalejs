    ...
    describe("complex real-world examples", () => {
        it("should create a scoreboard-like HUD", () => {
            const ui = new UIBuilder()
                ...
                .anchor({ height: 30 })
                .text("HytaleJS"), group({ id: "Separator1" }).anchor({ height: 1, top: 8 }).background({ color: "#2b3542" }), group({ id: "PlayerRow" })
                .layoutMode("Left")
                .anchor({ height: 22, top: 10 })
                .children(label({ id: "PlayerLabel" }).style({ fontSize: 13, textColor: "#8a9aab", verticalAlignment: "Center" }).anchor({ width: 80 }).text("Player:"), label({ id: "PlayerValue" }).style({ fontSize: 13, textColor: "#ffffff", verticalAlignment: "Center", renderBold: true }).flexWeight(1).text("Unknown"))))
                .build();
        ...
        it("should create a page with template overlay", () => {
            const ui = new UIBuilder()
                ...
                .template("$C.@DecoratedContainer")
                .anchor({ width: 400 })
                .children(group({ id: "Content" })
                .padding({ vertical: 32, horizontal: 45 })
                .children(label({ id: "title" }).style({ renderBold: true, textColor: "#FFFFFF" }).text("Default Title"), label({ id: "message" }).anchor({ top: 20 }).style({ textColor: "#94a7bb" }).text("Default Message"), group()
                .layoutMode("Center")
                .anchor({ top: 30 })
    ...
    describe("back button template", () => {
        it("should create back button reference", () => {
            const ui = new UIBuilder()
                ...
                .roots(group()
                .template("$C.@PageOverlay")
                .children(label({ text: "Content" })), group().template("$C.@BackButton"))
                .build();
            expect(ui).toContain("$C.@PageOverlay");
    ...
    describe("slot references", () => {
        ...
        it("should work with template slots in decorated container", () => {
            const ui = new UIBuilder()
                ...
                .root(group()
                .template("$C.@DecoratedContainer")
                .children(slot("Title").children(group().template("$C.@Title").param("Text", '"My Page"')), slot("Content")
                .padding({ full: 20 })

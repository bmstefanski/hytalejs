...
Some operations (like large block edits) are significantly faster and safer when they run on the **world thread**. Use `commands.registerWorld(...)` to execute a JS command on the world executor instead of a pooled JS thread.
...
- **Context pool contention:** the JS callback still needs a Graal context; if all contexts are busy, the world thread will wait.

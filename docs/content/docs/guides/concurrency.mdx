...
1. **Large Loops** - The `/sphere` command example:
```typescript
...
2. **Heavy Computations** - Complex calculations without yielding:
```typescript
...
3. **Sequential World Operations** - Multiple expensive calls in succession:
```typescript
...
_[Screenshot: /hytalejs poolstats output]_

**`/hytalejs tasks`** - Show operations waiting in queue

_[Screenshot: /hytalejs tasks output]_

## Technical Implementation Details

**Context Pool Implementation**

HytaleJS creates a pool of isolated GraalJS contexts at startup. Each context:
- Shares a common GraalJS Engine for compilation cache efficiency
- Has identical JavaScript bindings (`plugin`, `logger`, `commands`, etc.)
- Loads the same script code
- Stores callbacks in context-local storage (`__commandCallbacks__`, `__eventCallbacks__`, `__schedulerCallbacks__`)

**Callback Storage & Execution**

When a script registers a handler:
```typescript
...
1. The callback is stored in the calling context's `__commandCallbacks__` object
2. Command is registered with Hytale's command system
3. All other contexts in the pool also load the script and store their own callback references

When the command executes:
1. Hytale dispatches to Java handler
2. Handler acquires available context from pool
3. Looks up callback in that context's `__commandCallbacks__`
4. Executes callback
...
Operation Request
    ...
    ▼
┌─────────────────┐
│ Acquire Context │──── Blocks if all contexts busy
└────────┬────────┘
         │
         ▼
    ctx.enter() ──────── Marks context as owned by current thread
         │
         ▼
┌─────────────────┐
│ Execute JS Code │
└────────┬────────┘
         │
         ▼
    ctx.leave() ──────── Releases context ownership
         │
         ▼
┌─────────────────┐
│ Return to Pool  │
...
This follows GraalJS's threading requirements: contexts can be accessed from different threads, but only one thread at a time via `enter()`/`leave()`.
...
- [Multi-threaded Java-JavaScript Interoperability in GraalVM](https://medium.com/graalvm/multi-threaded-java-javascript-language-interoperability-in-graalvm-2f19c1f9c37b)
